...
class ExperimentDesign:
    ...
    def _set_dataloader(self):
        ...
        # if self.settings.dataset == "imagenet_mio":
        ...
        #         inception_color_preproccess(224)
        #     ])
        #     test_transform = transforms.Compose([
        #         bytes2image,
        #         lambda image: image.convert('RGB'),
    ...
    def _set_checkpoint(self):
        ...
        self.checkpoint = GuidedCheckPoint(self.settings.save_path, self.settings.quantization_k, self.logger)
        ...
        if self.settings.quan_retrain is not None:
            for i in range(len(self.settings.quantization_k)):
    ...
    def _set_model(self):
        if self.settings.dataset in ["cifar10", "cifar100"]:
            ...
            if self.settings.netType == "PreResNet":
                ...
                self.ori_model = md.custom.ResNetCifar(depth=self.settings.depth,
                                                       num_classes=self.settings.nClasses,
                                                       wide_factor=self.settings.wideFactor)
                for i in range(len(self.settings.quantization_k)):
                    self.quan_model.append(md.custom.QuanResNetCifar(depth=self.settings.depth,
                                                                     num_classes=self.settings.nClasses,
                                                                     wide_factor=self.settings.wideFactor))
            ...
            if self.settings.netType == "ResNet":
                self.ori_model = md.official.ResNet(self.settings.depth, self.settings.nClasses)
                for i in range(len(self.settings.quantization_k)):
                    self.quan_model.append(md.custom.QuanResNet(self.settings.depth, self.settings.nClasses))
            elif self.settings.netType == "PreResNet":
                self.ori_model = md.official.PreResNetImageNet(self.settings.depth, self.settings.nClasses)
                for i in range(len(self.settings.quantization_k)):
                    self.quan_model.append(md.official.PreResNetImageNet(self.settings.depth, self.settings.nClasses))
        ...
        if self.settings.netType == "PreResNet" and self.settings.dataset in ["cifar10", "cifar100"]:
            for i in range(len(self.settings.quantization_k)):
                for module in self.quan_model[i].modules():
                    # if isinstance(module, (md.official.PreResNet)):
                    # temp_conv = QConv2d(
                    #     k=self.settings.quantization_k[i],
                    ...
                    # module.relu = QReLU(self.settings.quantization_k[i], module.relu.inplace)
                    #
                    # temp_fc = QLinear(
                    #     k=self.settings.quantization_k[i],
                    ...
                    if isinstance(module, (md.official.PreBasicBlock)):
                        ...
                        module.relu1 = QReLU(self.settings.quantization_k[i], module.relu1.inplace)
                        module.relu2 = QReLU(self.settings.quantization_k[i], module.relu2.inplace)
            ...
            for i in range(len(self.settings.quantization_k)):
                for module in self.quan_model[i].modules():
                    if isinstance(module, md.official.PreBasicBlockImageNet):
                        ...
                        module.relu1 = QReLU(self.settings.quantization_k[i], module.relu1.inplace)
                        module.relu2 = QReLU(self.settings.quantization_k[i], module.relu2.inplace)
                        module.relu3 = QReLU(self.settings.quantization_k[i], module.relu3.inplace)
        elif self.settings.netType == "ResNet" and self.settings.dataset in ["imagenet", "imagenet_mio"]:
            for i in range(len(self.settings.quantization_k)):
                for module in self.quan_model[i].modules():
                    # if isinstance(module, (md.official.ResNet)):
                    # temp_conv = QConv2d(
            ...
            for i in range(len(self.settings.quantization_k)):
                for module in self.quan_model[i].modules():
                    if isinstance(module, torch.nn.Sequential) and len(module) == 18:
                        ...
                        # module[0] = temp_conv1
                        module[2] = QReLU(self.settings.quantization_k[i], module[2].inplace)
                        ...
                        module[4] = temp_conv2
                        module[6] = QReLU(self.settings.quantization_k[i], module[6].inplace)
                        ...
                        module[8] = temp_conv3
                        module[10] = QReLU(self.settings.quantization_k[i], module[10].inplace)
                        ...
                        module[13] = QReLU(self.settings.quantization_k[i], module[13].inplace)
                        ...
                        module[16] = QReLU(self.settings.quantization_k[i], module[16].inplace)
                    elif isinstance(module, torch.nn.Sequential) and len(module) == 7:
                        temp_fc1 = QLinear(
                            k=self.settings.quantization_k[i],
                        ...
                        module[3] = temp_fc2
                        # module[5] = QReLU(self.settings.quantization_k[i], module[5].inplace)
                        #
                        # temp_fc3 = QLinear(
                        #     k=self.settings.quantization_k[i],
        ...
        try:
            for epoch in range(self.start_epoch, self.settings.nEpochs):
                ...
                for j in range(len(self.quan_model)):
                    ...
                    self.logger.info(
                        "# ==>Quantized model_{} Best Result is: Top1 Error: {:.2f}, Top5 Error: {:.2f}".format(j, quan_best_top1[j], quan_best_top5[j]))
                    self.logger.info(
                        "# ==>Quantized model_{} Best Result is: Top1 Accuracy: {:.2f}, Top5 Accuracy: {:.2f}".format(j, 100 - round(quan_best_top1[j], 2), 100 - round(quan_best_top5[j], 2)))

                    if quan_best_flag:
                        # self.checkpoint.save_model(self.ori_model, best_flag=quan_best_flag, tag="Original_model")
                        self.checkpoint.save_model(self.quan_model[j], best_flag=quan_best_flag,
                                                   tag="Quantized_model_{}".format(j))
