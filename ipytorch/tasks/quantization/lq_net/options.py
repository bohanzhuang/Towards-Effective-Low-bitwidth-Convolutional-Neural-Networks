...
class Option(NetOption):
    ...
    def set_save_path(self):
        if self.netType in ["PreResNet", "ResNet"]:
            self.save_path = self.save_path + "log_{}{:d}_{}_bs{:d}_lr{:.4f}_TELCNN_baseline_qw{:d}_qa{:d}_epoch{}_{}/".format(
                self.netType, self.depth, self.dataset,
                self.batchSize, self.lr, self.qw,
                self.qa, self.nEpochs, self.experimentID)
        else:

            self.save_path = self.save_path + "log_{}_{}_bs{:d}_lr{:.4f}_TELCNN_baseline_qw{:d}_qa{:d}_epoch{}_{}/".format(
