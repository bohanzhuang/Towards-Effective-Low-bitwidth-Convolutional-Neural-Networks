...
class ExperimentDesign:
    ...
    def set_logger(self):
        logger = logging.getLogger('lqnet')
    ...
    def _set_model(self):
        if self.settings.dataset in ["cifar10", "cifar100"]:
            ...
            elif self.settings.netType == "VGGSmall":
                self.model = md.lqnet.VGG_SMALL(self.settings.qw, self.settings.qa, num_classes=self.settings.nClasses)
            else:
        ...
        if self.settings.netType == "PreResNet":
            for module in self.model.modules():
                ...
                if isinstance(module, (md.official.PreBasicBlock)):
                    ...
                    module.relu1 = QReLU(self.settings.quantization_k, module.relu1.inplace)
                    module.relu2 = QReLU(self.settings.quantization_k, module.relu2.inplace)
        elif self.settings.netType == "ResNet":
            for module in self.model.modules():
            ...
            for module in self.model.modules():
                if isinstance(module, torch.nn.Sequential) and len(module) == 18:
                    ...
                    # module[0] = temp_conv1
                    module[2] = QReLU(self.settings.quantization_k, module[2].inplace)
                    ...
                    module[4] = temp_conv2
                    module[6] = QReLU(self.settings.quantization_k, module[6].inplace)
                    ...
                    module[8] = temp_conv3
                    module[10] = QReLU(self.settings.quantization_k, module[10].inplace)
                    ...
                    module[13] = QReLU(self.settings.quantization_k, module[13].inplace)
                    ...
                    module[16] = QReLU(self.settings.quantization_k, module[16].inplace)
