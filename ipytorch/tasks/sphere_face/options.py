...
class Option(NetOption):
    def __init__(self, conf_path):
        ...
        self.dataset = self.conf['dataset']  # options: imagenet | cifar10 | cifar100 | imagenet100 | mnist | sphere
    ...
    def set_save_path(self):
        if self.netType in ["PreResNet", "ResNet"]:
            self.save_path = self.save_path + "log_{}{:d}_{}_bs{:d}_lr{:.3f}_epoch{}_{}/".format(
                self.netType, self.depth, self.dataset,
                self.batchSize, self.lr, self.nEpochs, self.experimentID)
        else:
            self.save_path = self.save_path + "log_{}_{}_bs{:d}_lr{:.3f}_epoch{}_{}/".format(
    ...
    def paramscheck(self, logger):
        if self.torch_version != "0.2.0":
            ...
            logger.info("|===>DrawNetwork is supported by PyTorch with version: 0.2.0. The used version is {}".format(
                self.torch_version))
