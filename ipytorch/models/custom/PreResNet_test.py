...
class PreBasicBlock(nn.Module):
    ...
    def __init__(self, in_plane, out_plane, stride=1, downsample=None, block_type="both_preact", conv1x1_flag=False):
        """
        ...
        if conv1x1_flag:
            self.conv1 = conv1x1(in_plane, out_plane, stride)
            self.conv2 = conv1x1(out_plane, out_plane)
        else:
    ...
    def forward(self, x):
        ...
        if x.size(1) != residual.size(1):
            zero_pad = torch.zeros(x.size(0), x.size(1)-residual.size(1), x.size(2), x.size(3))
            zero_pad = Variable(zero_pad.type(x.data.type(), True))
            residual = torch.cat((residual, zero_pad), 1)
        out = x + residual
        return out


class PreResNet_Test(nn.Module):
    """
    define PreResNet on small data sets
    """

    def __init__(self, depth, wide_factor=1, num_classes=10, max_conv=None):
        """
        ...
        """
        super(PreResNet_Test, self).__init__()
    ...
    def _make_layer(self, block, out_plane, n_blocks, stride=1):
        ...
        layers = []
        if self.max_conv is not None and self.block_count >= self.max_conv:
            conv1x1_flag = True
        else:
            conv1x1_flag = False

        layers.append(block(self.in_plane, out_plane, stride,
                            downsample, block_type="half_preact", conv1x1_flag=conv1x1_flag))
        self.block_count += 1
        ...
        for i in range(1, n_blocks):
            ...
            layers.append(block(self.in_plane, out_plane, conv1x1_flag=conv1x1_flag))
            self.block_count += 1
