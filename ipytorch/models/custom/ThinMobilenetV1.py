...
class ThinMobileNetV1(nn.Module):
    """
    MobileNetV1 on cifar (small datasets)
    """

    def __init__(self, num_classes=1000, wide_scale=1.0, width=[]):
        super(ThinMobileNetV1, self).__init__()

        # define network structure

        # self.layer_width = np.array([32, 64, 128, 256, 512, 1024])
        self.layer_width = np.array(width)
        self.layer_width = np.ceil(self.layer_width * wide_scale)
        self.layer_width = self.layer_width.astype(int)
        self.segment_layer = [1, 1, 2, 2, 6, 1] # number of layers in each segment
        self.down_sample = [1, 2, 3, 4] # the place of down_sample, related to segment_layer
