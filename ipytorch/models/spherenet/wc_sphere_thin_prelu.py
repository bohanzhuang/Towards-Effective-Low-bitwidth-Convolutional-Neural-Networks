...
def conv3x3(in_planes, out_planes, stride=1, rate=0.):
    ...
    return prune.wcConv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=False, rate=rate)
...
class wcSphereBlock(nn.Module):
    ...
    def __init__(self, planes, rate=0.):
        ...
        self.conv1 = conv3x3(planes, planes, rate=rate)
        self.relu = nn.PReLU(planes)
        self.conv2 = conv3x3(planes, planes, rate=rate)
...
class wcSphereNet(nn.Module):
    ...
    def __init__(self, depth, num_output=10572, num_features=512, margin_inner_product_type='quadruple', rate=0.):
        ...
        self.conv2 = prune.wcConv2d(30, 93, kernel_size=3, stride=2, padding=1, rate=rate)
        # self.relu2 = nn.ReLU()
        self.relu2 = nn.PReLU(93)
        self.layer2 = self._make_layer(block, 93, layers[1], stride=2)
        
        self.conv3 = prune.wcConv2d(93, 242, kernel_size=3, stride=2, padding=1, rate=rate)
        # self.relu3 = nn.ReLU()
        self.relu3 = nn.PReLU(242)
        self.layer3 = self._make_layer(block, 242, layers[2], stride=2)
        
        self.conv4 = prune.wcConv2d(242, 256, kernel_size=3, stride=2, padding=1, rate=rate)
        # self.relu4 = nn.ReLU()
