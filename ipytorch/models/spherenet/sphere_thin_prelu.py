...
class SphereNet(nn.Module):
    ...
    def __init__(self, depth, num_output=10572, num_features=512, margin_inner_product_type='quadruple'):
        ...
        self.gamma = 0.12
        self.power = 1.
        self.width = [18, 50, 98, 512]

        self.depth = depth
        block = SphereBlock
        # define network structure
        self.conv1 = nn.Conv2d(3, self.width[0], kernel_size=3, stride=2, padding=1)
        # self.relu1 = nn.ReLU()
        self.relu1 = nn.PReLU(self.width[0])
        self.layer1 = self._make_layer(block, self.width[0], layers[0])

        self.conv2 = nn.Conv2d(self.width[0], self.width[1], kernel_size=3, stride=2, padding=1)
        # self.relu2 = nn.ReLU()
        self.relu2 = nn.PReLU(self.width[1])
        self.layer2 = self._make_layer(block, self.width[1], layers[1], stride=2)
        
        self.conv3 = nn.Conv2d(self.width[1], self.width[2], kernel_size=3, stride=2, padding=1)
        # self.relu3 = nn.ReLU()
        self.relu3 = nn.PReLU(self.width[2])
        self.layer3 = self._make_layer(block, self.width[2], layers[2], stride=2)
        
        self.conv4 = nn.Conv2d(self.width[2], self.width[3], kernel_size=3, stride=2, padding=1)
        # self.relu4 = nn.ReLU()
        self.relu4 = nn.PReLU(self.width[3])
        self.layer4 = self._make_layer(block, self.width[3], layers[3], stride=2)
        # self.pooling = nn.MaxPool2d(kernel_size=(7, 6))
        self.fc = nn.Linear(self.width[3] * 7 * 6, num_features)
