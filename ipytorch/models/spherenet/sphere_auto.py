...
class SphereNet(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        if target is not None:
            ...
            # normalize weight
            self.margin_inner_product_weight.data.copy_(F.normalize(self.margin_inner_product_weight, p=2, dim=0).data)

            # common variables
            # x_norm_ = |x|
            x_norm = torch.norm(x, 2, 1)
            # cos_theta = xw'/|x|
            xw = x.matmul(self.margin_inner_product_weight)
            ...
            feature = x.clone()
            x = x.matmul(self.margin_inner_product_weight)
            ...
            if self.margin_inner_product_type == 'single':
                ...
                # + lambda * x'w
                xw = feature.matmul(self.margin_inner_product_weight)
