...
def quantization(x, k, val):
    n = (torch.pow(2, k) - 1) / val
    # n = 2 ** k - 1
    return RoundFunction.apply(x, n)


def quantization_activation(x, k, val):
    activation = normalization_on_activations(x, val)
    return quantization(activation, k, val)
...
def normalization_on_activations(x, val):
    return torch.clamp(x, 0, val)
...
class QConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        if self.quantized:
            ...
            quantized_weight = 2 * quantization(normalized_weight, self.k, 1) - 1
...
class QReLU(nn.ReLU):
    ...
    def forward(self, input):
        ...
        out = normalization_on_activations(input, 2)
        if self.quantized:
            return quantization(out, self.k, 2)
        else:
            return out
