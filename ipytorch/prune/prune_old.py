...
class FilterPrune(object):
    ...
    def feature_extract(self, x, y, layer):
        ...
        if isinstance(layer, nn.Conv2d):
            ...
            # compute x of every channel
            x_n = torch.LongTensor(
                range(x.size(0))).cuda().repeat(self.locations)
    ...
    def get_thin_params(self, layer, dim=0):
        ...
        if isinstance(layer, nn.Conv2d) or isinstance(layer, twnConv2d):
        ...
        elif isinstance(layer, twnConv2d):
            thin_float_weight = layer.weight.data.index_select(
                dim, self.select_channels)
            thin_ternary_weight = layer.weight_ternary.data.index_select(
                dim, self.select_channels)
            ...
            if layer.bias is not None:
                ...
                thin_bias = None
            thin_weight = (thin_float_weight, thin_ternary_weight)
            ...
            if dim == 0:
                ...
                if layer.bias is not None:
                    ...
                    thin_bias = None
         elif isinstance(layer, twnLinear):
                if dim == 0:
                thin_float_weight = layer.weight.data.index_select(
                    dim, self.select_channels)
                ...
                fat_float_weight = layer.weight.data.view(layer.weight.size(
                    0), layer.weight.size(1) / (self.h * self.w), self.h, self.w)

                thin_float_weight = fat_float_weight.index_select(
                    dim, self.select_channels)

                thin_float_weight = thin_float_weight.view(
                    layer.weight.size(0), -1)

                #-------------------------------
                fat_ternary_weight = layer.weight_ternary.data.view(layer.weight.size(
                    0), layer.weight.size(1) / (self.h * self.w), self.h, self.w)

                thin_ternary_weight = fat_ternary_weight.index_select(
                    dim, self.select_channels)

                thin_ternary_weight = thin_ternary_weight.view(
                    layer.weight.size(0), -1)
...
def replace_segment(model, index, init_weight, init_bias=None, scale=None):
    ...
    for i, _ in enumerate(model):
        if i == index:
            """if isinstance(old_layer, twnConv2d) or isinstance(old_layer, twnLinear):
                init_float_weight = init_weight[0]
                init_ternary_weight = init_weight[1]
                if scale is not None:
                    init_float_weight = init_float_weight * scale
                    ternary_scale = 1.0
                    # ternary_scale = scale[0, :, 0, 0].mean()
                else:
                    ternary_scale = 1.0
                ...
                if isinstance(old_layer, twnConv2d):
                    ...
                    new_layer.weight_alpha.data.copy_(old_layer.weight_alpha.data*ternary_scale)
                    new_layer.weight.data.copy_(init_float_weight)
                    new_layer.weight_ternary.data.copy_(init_ternary_weight)
            ...
            if isinstance(old_layer, nn.Conv2d) or isinstance(old_layer, nn.Linear) or isinstance(old_layer, nn.BatchNorm2d):
                if scale is not None:
...
def replace_layer(old_layer, init_weight, init_bias=None, scale=None):
    ...
    elif isinstance(old_layer, nn.Conv2d) or isinstance(old_layer, nn.Linear) or isinstance(old_layer, nn.BatchNorm2d):
        if scale is not None:
            init_weight = init_weight * scale
...
# -------------------------------------------------------------------------------------------------------------
class ModelPrune:
    def __init__(self, model, trainer, net_type, prune_ratio=0.5):
        self.model = model
    ...
    def run(self):
        ...
        if self.net_type in ["LeNet5", "VGG", "AlexNet"]:
            ...
            segment_count = 0
            while segment_count < len(self.segment_prune):
                # feature extraction
                ...
                # fine-tuning
                train_top1error, train_loss, train_top5error = self.trainer.train(
                    epoch=0)
                test_top1error, test_loss, test_top5error = self.trainer.test(
                    epoch=0)
            ...
            while segment_count < 5:
                ...
                # self.trainer.train(0, self.train_loader)
                # self.trainer.test(0, self.test_loader)
