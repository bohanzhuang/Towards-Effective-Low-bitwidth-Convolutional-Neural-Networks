...
from . import transform as auxtransform
...
class DataLoader(object):
    ...
    def __init__(self, dataset, batch_size, n_threads=4,
        ...
        if self.dataset in ["cifar10", "cifar100"]:
            ...
            self.train_loader, self.test_loader = self.svhn(
                dataset=self.dataset)

        elif self.dataset in ["imagenet", "imagenet100", "thi_imgnet", "imagenet_mio"]:
    ...
    def casia(self):
        ...
        img_source = os.path.join(
            self.data_path,
            'Face/FaceRecognition/CASIA-WebFace/training/CASIA-WebFace-112X96-186.txt')
    ...
    def lfw(self):
        ...
        lfw_pairs = os.path.join(
            self.data_path, 'Face/FaceRecognition/CASIA-WebFace/testing/pairs.txt')
        lfw_dir = os.path.join(
            self.data_path, 'Face/FaceRecognition/CASIA-WebFace/testing/lfw-112X96')
        ...
        test_dataset = LFWDataset(
            lfw_dir, lfw_pairs, transform=test_transforms, logger=self.logger)
    ...
    def imagenet(self, dataset="imagenet"):
        ...
        elif dataset == "imagenet_mio":
            dataset_path = os.path.join(self.data_path, "imagenet_mio")
        ...
        if dataset == "imagenet_mio":
            train_loader = torch.utils.data.DataLoader(
                MioDataset(root=traindir,
                           sampler=lambda id_, size: 0,
                           ...
                           target_transform=torchvision.transforms.Compose([
                               lambda metadata: struct.unpack("<iII", metadata)[0],
                           ])),
    ...
    def svhn(self, dataset="svhn"):
        data_root = os.path.join(self.data_path, "svhn")
        train_transform = transforms.Compose([
        ...
        train_dataset = dsets.SVHN(root=data_root,
                                   ...
                                   download=False,
                                   transform=train_transform)
        extra_dataset = dsets.SVHN(root=data_root,
                                   split='extra',
                                   download=False,
                                   transform=train_transform)
        combine_dataset = ComcateSVHNDataset(train_dataset, extra_dataset)
        test_dataset = dsets.SVHN(root=data_root,
                                  split='test',
                                  download=False,
                                  transform=test_transform)

        train_loader = torch.utils.data.DataLoader(dataset=combine_dataset,
                                                   batch_size=self.batch_size,
                                                   shuffle=True,
                                                   pin_memory=True,
                                                   num_workers=self.n_threads)
...
class ComcateSVHNDataset(torch.utils.data.Dataset):
    def __init__(self, train_dataset, extra_dataset):
        self.train_dataset = train_dataset
        self.extra_dataset = extra_dataset
        self.data = np.concatenate((self.train_dataset.data, self.extra_dataset.data), axis=0)
        self.labels = np.concatenate((self.train_dataset.labels, self.extra_dataset.labels), axis=0)
        self.transform = self.train_dataset.transform
        self.target_transform = self.train_dataset.target_transform
...
class PartDataLoader(DataLoader):
    ...
    def __init__(self, dataset, batch_size, n_threads=4,
                 ten_crop=False, data_path='/home/dataset/', data_ratio=1.0, logger=None):
...
class LFWDataset(data.Dataset):
    ...
    def __init__(self, lfw_dir, lfw_pairs_path, transform=None, target_transform=None, logger=None):
        ...
        with open(lfw_pairs_path, 'r') as f:
            for i, line in enumerate(f.readlines()[1:]):
                ...
                if len(strings) == 3:
                    ...
                    self.logger.info("Lfw pairs.txt file error!")
                    exit(-1)
                pairs.append(pair)
        return pairs
